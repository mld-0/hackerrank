...
{
	if (accumulate(arr.begin()+1, arr.end(), 0) == 0) {
		return "YES";
	}
	if (accumulate(arr.begin(), arr.end()-1, 0) == 0) {
		return "YES";
	}
	int sum_l = 0;
	int sum_r = accumulate(arr.begin()+1, arr.end(), 0);
...
int main()
{
	vector<vector<int>> input_values = { {1,2,3}, {1,2,3,3}, {1,1,4,1,1}, {2,0,0,0}, {0,0,2,0}, };
	vector<string> input_checks = { "NO", "YES", "YES", "YES", "YES", };
